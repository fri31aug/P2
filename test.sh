#!/bin/bash
make clean
make
gcc userspace.c -o userspace
clear;

free -h;
echo "---------------------- Test Case: $1 -----------------------------"
sudo dmesg -c > /dev/null

if [[ $1 -le 2 ]]; then

    ./userspace $1 &
    pid=$!

    echo "Loading the test kernel module ..."
    sudo insmod memory_manager.ko pid=$pid

    count=1
    while [ $count -le 3 ]
    do
        sleep 10
        vmRss=( $(sudo cat /proc/$pid/smaps | grep Rss: | awk '{print $2}' | awk '{ SUM += $1} END { print SUM }') )
        vmSwap=( $(sudo cat /proc/$pid/smaps | grep Swap: | awk '{print $2}' | awk '{ SUM += $1} END { print SUM }') )
        vmRssMB=$((vmRss / 1024))
        vmSwapMB=$((vmSwap / 1024))
        echo "[TESTSCRIPT] [PID-$pid] Reported [RSS:$vmRssMB MB] [Swap:$vmSwapMB MB]"
        ((count++))
    done

    echo "Removing the test kernel module ..."
    sudo rmmod memory_manager

    echo "The following is the dmesg output"
    sudo dmesg | grep $pid
    echo "----------------------------------------------------------------------------------";
    echo ""
else
    # Swap Test Case
    scalar=4 # Start with 4 GB of memory pressure
    one_gb=1073741824
    index=1
    while [ $index -le 3 ]; do
        mem_size_gb=$(echo "scale=2; $scalar" | bc) # Apply memory pressure

        echo "---------------------------------------------------------------------";
        echo "------- SWAP Test Case iteration: $index with Memory Size : $mem_size_gb GB ------"
        echo "---------------------------------------------------------------------";
        ./userspace $1 $scalar &
        pid=$!

        echo "Loading the test kernel module ..."
        sudo insmod memory_manager.ko pid=$pid
        count=1

        while [ $count -le 3 ]
        do
            sleep 10
            vmRss=( $(sudo cat /proc/$pid/smaps | grep Rss: | awk '{print $2}' | awk '{ SUM += $1} END { print SUM }') )
            vmSwap=( $(sudo cat /proc/$pid/smaps | grep Swap: | awk '{print $2}' | awk '{ SUM += $1} END { print SUM }') )
            vmRssMB=$((vmRss / 1024))
            vmSwapMB=$((vmSwap / 1024))
            echo "[TESTSCRIPT] [PID-$pid] Reported [RSS:$vmRssMB MB] [Swap:$vmSwapMB MB]"
            ((count++))
        done

        echo "Removing the test kernel module ..."
        sudo rmmod memory_manager

        echo "The following is the dmesg output"
        sudo dmesg | grep $pid
        echo "----------------------------------------------------------------------------------";
        echo ""
        killall userspace
        ((index++))
        ((scalar++)) # Increase the memory pressure by 1 GB for each iteration
    done

fi
killall userspace
rm userspace
